...
pub fn sync_session_usage(
    ...
    // 同步 PiAgent 使用数据
    match crate::services::session_usage_pi_agent::sync_pi_agent_usage(&state.db) {
        Ok(pi_agent_result) => {
            result.imported += pi_agent_result.imported;
            result.skipped += pi_agent_result.skipped;
            result.files_scanned += pi_agent_result.files_scanned;
            result.errors.extend(pi_agent_result.errors);
        }
        Err(e) => {
            result.errors.push(format!("PiAgent 同步失败: {e}"));
        }
    }
...
/// 删除失败请求日志（非 2xx 状态码）
#[tauri::command]
pub fn delete_failed_request_logs(
    state: State<'_, AppState>,
) -> Result<u64, AppError> {
    let conn = crate::database::lock_conn!(state.db.conn);
    let deleted = conn
        .execute(
            "DELETE FROM proxy_request_logs WHERE status_code < 200 OR status_code >= 300",
            [],
        )
        .map_err(|e| AppError::Database(format!("删除失败请求日志失败: {e}")))?;
    log::info!("已删除 {deleted} 条失败请求日志");
    Ok(deleted as u64)
}
