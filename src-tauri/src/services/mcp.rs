...
impl McpService {
    ...
    fn sync_server_to_app_no_config(server: &McpServer, app: &AppType) -> Result<(), AppError> {
        match app {
            ...
            AppType::PiAgent => {
                // Pi-Agent doesn't support MCP, skip
                log::debug!("Pi-Agent MCP support is not implemented, skipping sync");
            }
        }
        Ok(())
    }
    ...
    fn remove_server_from_app(_state: &AppState, id: &str, app: &AppType) -> Result<(), AppError> {
        match app {
            ...
            AppType::PiAgent => {
                // Pi-Agent doesn't support MCP, skip
                log::debug!("Pi-Agent MCP support is not implemented, skipping remove");
            }
        }
        Ok(())
    }
