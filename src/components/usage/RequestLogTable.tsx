...
import { useDeleteFailedRequestLogs } from "@/lib/query/usage";
import {
  getFreshInputTokens,
  isUnpricedUsage,
  type LogFilters,
...
export function RequestLogTable({
  ...
  const deleteFailedLogs = useDeleteFailedRequestLogs();
      ...
      <div className="rounded-lg border bg-card/50 p-2 backdrop-blur-sm">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <Button
            ...
            onClick={() => deleteFailedLogs.mutate()}
            disabled={deleteFailedLogs.isPending}
            title={t("usage.deleteFailedLogs", "清理失败请求")}
          >
            <Trash2 className="h-3.5 w-3.5" />
            {t("usage.deleteFailedLogs", "清理失败请求")}
          </Button>
